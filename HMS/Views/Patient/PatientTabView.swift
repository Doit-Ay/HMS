    ...
    private func fetchUpcomingAppointments() async {
        ...
        do {
            ...
            let sortedApps = fetchedApps.sorted { app1, app2 in
                if app1.date != app2.date { return app1.date < app2.date }
...
struct UpcomingAppointmentCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                HStack(spacing: 12) {
                    Circle()
                        ...
                        .overlay(Image(systemName: "stethoscope").foregroundColor(AppTheme.primaryDark))
            ...
            HStack(spacing: 24) {
                HStack(spacing: 6) {
                    Image(systemName: "calendar").foregroundColor(AppTheme.primary).font(.system(size: 14))
                    Text(formatDate(appointment.date))
                        .font(.system(size: 14, weight: .medium, design: .rounded))
                        .foregroundColor(AppTheme.textPrimary)
                }
                HStack(spacing: 6) {
                    Image(systemName: "clock.fill").foregroundColor(AppTheme.primary).font(.system(size: 14))
...
struct VitalRow: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(title).font(.system(size: 14, weight: .semibold, design: .rounded)).foregroundColor(AppTheme.textPrimary)
                Text(value + (unit.isEmpty ? "" : " \(unit)")).font(.system(size: 13)).foregroundColor(AppTheme.textSecondary)
            }
            Spacer()
        }
        .padding()
        ...
        HStack {
            VStack(alignment: .leading, spacing: 6) {
                Text(Date().formatted(date: .abbreviated, time: .omitted))
                    ...
                    .textCase(.uppercase)
                HStack(spacing: 6) {
                    Text("Hi,").font(.system(size: 28, weight: .regular, design: .rounded)).foregroundColor(AppTheme.textSecondary)
                    Text(session.currentUser?.fullName.components(separatedBy: " ").first ?? "Patient")
                        .font(.system(size: 28, weight: .bold, design: .rounded)).foregroundColor(AppTheme.textPrimary)
                }
            }
            Spacer()
